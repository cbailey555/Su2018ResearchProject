...
pub enum LibError {
    ...
    EmptyZeroError { field: String },
    #[fail(display = "{}\n", contents)]
    CustomError { contents: String },
    #[fail(
        display = "User {} already exists in our user database. Cannot create duplicate user.", user
    )]
    UserExistsError { user: String },
    #[fail(display = "{} does not exist in {}", contents, structure)]
    NExistKeyError { contents: String, structure: String },
    #[fail(
        display = "Experienced integer overlow at: {}. Offending integers were {} and {} for integer size {}",
        origin,
        fst,
        snd,
        intsize
    ...
    #[fail(
        display = "Experienced integer underflow at: {}. Offending values were {} and {}, for type {}",
        origin,
        fst,
        snd,
        intsize
    ...
    #[fail(
        display = "Received a CLI type error (the one from swth_cli_libv2); the only places that's used here are in the to_swth_transaction and to_cli_req methods for the wrapper type"
    )]
    CliError,
}
